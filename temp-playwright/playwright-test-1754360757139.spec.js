...
try {
  // Use the exact selector that contains the total number of chefs available
  const targetElement = page.locator('.total-chefs').first(); // Assuming '.total-chefs' is the correct class for total chefs
  const text = await targetElement.textContent();
  
  // Extract the specific number/data needed
  const result = text.match(/\d+/)?.[0]; // Extract first number found
  
  console.log('RESULT::', { extractedData: result, fullText: text });
  await sendSnapshot(page, 'Data extracted successfully');
} catch (error) {
  console.log('RESULT::', { error: 'Failed to extract data', details: error.message });
